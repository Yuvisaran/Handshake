...
class AddPropertyStep1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showIcons: false,
  ...
  deleteBannerImage = (img, e) => {
    ...
    this.setState(this.state.testArray);
    this.setState(this.state.Allimage_Files);
    this.setState({ bannerimage: this.state.testArray[0] });
  ...
  newFileChoosen(files) {
    ...
    for (let i = 0; i < files.base64.length; i++) {
      ...
      self.state.testArray.push(files.base64[i]);
    }
    ...
    this.setState({ bannerimage: self.state.testArray[0] });
    this.setState({ bannerimageIndex: 0 });
  }
  ...
  submitValidations(event) {
    ...
    if (event.address) {
      ...
      if (event.address.zipcode) {
        if (!validator.isNumeric(event.address.zipcode)) {
          errors.zipcode = 'Zipcode should be numeric';
          address.zipcode = '';
        }
    ...
    if (event.property_type.p_type != 'Lot / Land') {
      ...
      if (event.num_of_bedrooms) {
        ...
        if (!(event.num_of_bedrooms > 0 && event.num_of_bedrooms <= 99)) {
          errors.num_of_bedrooms = "Bedrooms Should be min 1 and max 2 digits";
          self.setState({ num_of_bedrooms: '' });
        }
      ...
      if (event.num_of_bathrooms) {
        if (!(event.num_of_bathrooms > 0 && event.num_of_bathrooms <= 99)) {
          errors.num_of_bathrooms = 'Bathrooms Should be min 1 and max 2 digits';
          self.setState({ num_of_bathrooms: '' });
        }
        if (event.num_of_bathrooms) {
      ...
      if (event.square_footage) {
        if (!(event.square_footage.length >= 3 && event.square_footage.length <= 6)) {
          errors.square_footage = 'SquareFootage Should be min 3 and max 6 digits';
          self.setState({ square_footage: '' });
        }
    ...
    if (event.property_type.p_type == 'Lot / Land') {
      ...
      if (!(event.num_of_acres.length >= 3 && event.num_of_acres.length <= 6)) {
        errors.num_of_acres = 'SquareFootage Should be min 3 and max 6 digits';
        self.setState({ num_of_acres: '' });
      }
    ...
    if (event.list_price) {
      ...
      else if ((event.list_price.length >= 5 && event.list_price.length <= 10)) {
        errors.list_price = 'ListPrice Should be min 5 and max 10 digits';
        self.setState({ list_price: '' });
      }
    }
  ...
  render() {
    console.log("ERRORS MESSAGE", this.state.errors, this.state);
    const { address, submitted, propertytype_List, errors, property_type, prop_attribute, records_per_page, current_page } = this.state;
            ...
            <div className="row">
              <div className="col-lg-4 col-md-4 col-sm-12 col-12">
                <div className="property_address">
                  ...
                  <p className="sub_head">Address<sup style={{ color: "red" }}>*</sup></p>
                  <label>Street Address <sup style={{ color: "red", top: "5px" }}>*</sup></label>
                  <input
                    type="text"
                    className="width370"
                    name="street_address"
                  ...
                  <label>Apt / Unit <sup style={{ color: "red", top: "5px" }}>*</sup></label>
                  <input
                    type="text"
                    className="width170"
                    name="apt"
                  ...
                  <div className="display-inline">
                    <div className="flex-column margR10">
                      <label>City <sup style={{ color: "red", top: "5px" }}>*</sup></label>
                      <input
                        type="text"
                        className="width170"
                        name="city"
                    ...
                    <div className="flex-column margR10 pos_relative">
                      <label>State <sup style={{ color: "red", top: "5px" }}>*</sup></label>
                    ...
                    <div className="flex-column">
                      <label>Zipcode <sup style={{ color: "red", top: "5px" }}>*</sup></label>
                      <input
                        type="number"
                        className="width110"
                        name="zipcode"
              ...
              <div className="col-lg-8 col-md-8 col-sm-12 col-12">
                <div className="property_pic_selection pos_relative">
                  {this.state.testArray.length > 0 ? <img src={this.state.bannerimage} alt="Profile" onMouseOver={this.onHover} /> : <a onMouseOver={this.onHover}></a>
                  ...
                  <div className="property_pic_upload">
                    <div className="row">
                      <div className="col-xl-6 col-md-7 col-sm-7 col-12 pl-2 pr-2 pic_upload_note">
                        <p className="big_head">Upload Photos from Your Computer<sup style={{ color: "red", top: "5px" }}>*</sup></p>
                  ...
                  {submitted && !this.state.image &&
                    <label className="errormessage" style={{ color: 'red' }}>{errors.image}</label>
                  }
          ...
          <div className="imageSliderShow">
            ...
            {this.state.testArray.length > 0 && this.state.testArray.map((data, index) => {
              if (index >= ((this.state.current_page - 1) * this.state.records_per_page) && index < (this.state.current_page * this.state.records_per_page)) {
                ...
                return (
                  <span key={index}>
                    <img src={data} name={'pra' + index} alt="Profile" className="slideerImage" onClick={() => this.changeImage(data, index)} />
                    <span className="delete"><img src={trash} alt="delete" href='' onClick={() => this.deleteImage(data, index)} /></span>
                  </span>
                )
              }
            })}
            ...
            <div className="col-lg-8 col-md-12 col-sm-12 col-12 center-align">
              <div className="row">
                ...
                <div className="col-lg-4 col-md-4 col-sm-12 col-12 flex-column display-flex pl-0 respons_padL15">
                  <p className="label_select">Select One <sup style={{ color: "red", top: "5px" }}>*</sup></p>
                  {
                    propertytype_List.map((data, index) => {
                      return (
                        <label className="custom_checkbox" key={index}>
                          {data.p_type}
                          <input type="radio" name="p_type" value={data.p_type} onChange={data => this.handlepropType(data, index)} />
              ...
              <div className="row margT60">
                <p className="property_type respons_padL15">Property Details <sup style={{ color: "red", top: "5px" }}>*</sup></p>
                  ...
                  <div className="col-md-12 noPadLR prop_detail_border respons_margL15 margT10">
                    <div className="col-md-4">
                      <input
                        ...
                        min="3" max="99" /><p>NO. OF BEDROOMS <sup style={{ color: "red", top: "5px" }}>*</sup></p>

                      {submitted && !this.state.num_of_bedrooms &&
                        <label className="errormessage1 deferError" style={{ color: 'red' }}>{errors.num_of_bedrooms}</label>
                      }
                    </div>
                    <div>
                      <input type="number"
                        ...
                        onChange={this.handleUserInput} min="3" max="99" /><p>NO. OF BATHROOMS <sup style={{ color: "red", top: "5px" }}>*</sup></p>
                      {submitted && !this.state.num_of_bathrooms &&
                        <label className="errormessage1 deferError" style={{ color: 'red' }}>{errors.num_of_bathrooms}</label>
                      }
                    </div>
                    <div>
                      <input type="number"
                        ...
                        onChange={this.handleUserInput} min="111" max="999999" /><p>SQUARE FOOTAGE <sup style={{ color: "red", top: "5px" }}>*</sup></p>
                      {submitted && !this.state.square_footage &&
                        <label className="errormessage1 deferError" style={{ color: 'red' }}>{errors.square_footage}</label>
                      }
                    </div>
                  ...
                  <div className="col-md-12 noPadLR prop_detail_border respons_margL15 margT10">
                    <div>
                      <input type="number"
                        ...
                        onChange={this.handleUserInput} min="111" max="999999" /><p>NO. OF ACRES <sup style={{ color: "red", top: "5px" }}>*</sup></p>

                      {submitted && !this.state.num_of_acres &&
                        <label className="errormessage1" style={{ color: 'red' }}>{errors.num_of_acres}</label>
                      }
                    </div>
              ...
              <div className="row margT60 margBT60">
                <div className="d-flex flex-row final_price_property">
                  ...
                  <div className="">
                    <p className="property_type">Preferences <sup style={{ color: "red", top: "5px" }}>*</sup></p>
                    <div className="radio">
                      <label className="custom_checkbox">
            ...
            <div className="row">
              <div className="col-lg-9 col-md-9 col-sm-12 col-12 custom_property_footer ">
                <button
                  ...
                  disabled={address.street_address == "" || address.apt == "" || address.city == "" || address.state == "" || address.zipcode == "" ||
                    this.state.property_type.id == "" || this.state.num_of_bedrooms == "" || this.state.num_of_bathrooms == "" || this.state.square_footage == "" ||
                    this.state.selectedOption == "" || this.state.list_price == ""}
                  onClick={() =>{
                    this.firstStepCompleted();
                    this.setState({loading : true})
                  }} >Next</button>
                {console.log(this.state.Allimage_Files.length)}
              </div>
            </div>
          </div>
        </div>
